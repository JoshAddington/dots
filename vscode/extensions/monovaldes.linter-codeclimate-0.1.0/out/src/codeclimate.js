...
class CodeClimateLintingProvider {
    ...
    doCClint() {
        ...
        if (!('linter_codeclimate' in conf && conf.linter_codeclimate.enabled) || vscode.window.activeTextEditor === null || path === undefined) {
            return;
        }
        ...
        let resultStream = '';
        try {
            const ls = cp.spawn('codeclimate', ['analyze', '-f', 'json', exec_path], { cwd: vscode.workspace.rootPath, detached: true, stdio: ['ignore', 'pipe', 'pipe'] });
